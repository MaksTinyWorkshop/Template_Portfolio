import { QuickAction, StatsCard } from "@/components/admin/DashboardStats";
...
async function getStats(): Promise<DashboardStats> {
  const projectsResult = await listMDXFiles("project");
  const postsResult = await listMDXFiles("post");

  interface ContentItem {
    status?: "draft" | "scheduled" | "published";
  }

  const projects = (projectsResult.success ? projectsResult.data || [] : []) as ContentItem[];
  const posts = (postsResult.success ? postsResult.data || [] : []) as ContentItem[];
