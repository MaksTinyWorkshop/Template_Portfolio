...
export async function generateAuthToken(): Promise<string> {
  ...
  if (secret.length < MIN_SECRET_LENGTH) {
    console.warn(
      `⚠️  SÉCURITÉ: Le secret a seulement ${secret.length} caractères. Recommandé: minimum ${MIN_SECRET_LENGTH * 2} caractères (${MIN_SECRET_LENGTH} bytes hex). Générer avec: openssl rand -hex 32`,
    );
  }
