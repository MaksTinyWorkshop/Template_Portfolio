...
async function navigateProtectedRoute(page: Page, route: string) {
  try {
    await page.goto(route, { waitUntil: "domcontentloaded" });
  } catch {
    // Some browsers interrupt the navigation when middleware redirects to /admin.
  }
}
...
test.describe("Tests d'Authentification Négatifs @p0 @security @auth", () => {
  ...
  test("should reject login with wrong password and show error message", async ({
    ...
    await page.setExtraHTTPHeaders({
      "x-forwarded-for": getUniqueTestIp(testInfo.title, testInfo.file, testInfo.project.name),
    });
    ...
    await expect(page.getByRole("heading", { name: /Dashboard|Ton Dashboard/i })).not.toBeVisible();
    ...
    for (const selector of possibleErrorSelectors) {
      try {
        ...
        errorFound = true;
        console.log("✅ Message d'erreur détecté:", await selector.textContent());
        break;
      } catch {
   ...
   */
  test("should allow login with correct password (control test)", async ({ page }, testInfo) => {
    await page.setExtraHTTPHeaders({
      "x-forwarded-for": getUniqueTestIp(testInfo.title, testInfo.file, testInfo.project.name),
    });
    await navigateToAdminLogin(page);
   ...
   */
  test("should block unauthenticated access to admin routes", async ({ page }, testInfo) => {
    await page.setExtraHTTPHeaders({
      "x-forwarded-for": getUniqueTestIp(testInfo.title, testInfo.file, testInfo.project.name),
    });
    ...
    for (const route of protectedRoutes) {
      ...
      const isRedirectedToLogin = currentUrl.includes("/admin") && !currentUrl.includes("/admin/");
   ...
   */
  test("should handle session expiration gracefully", async ({ page, context }, testInfo) => {
    await page.setExtraHTTPHeaders({
      "x-forwarded-for": getUniqueTestIp(testInfo.title, testInfo.file, testInfo.project.name),
    });
    ...
    await navigateProtectedRoute(page, "/admin/projects");
    ...
    await navigateProtectedRoute(page, "/admin/blog");

    // Vérifier qu'on est redirigé vers le login ou qu'un formulaire de login est affiché
    const currentUrl = page.url();
    const isOnLoginPage = currentUrl.endsWith("/admin") || currentUrl.includes("/admin?");
    const hasLoginForm = await page
      .getByLabel("Mot de passe")
      .isVisible()
      .catch(() => false);
    ...
    if (isOnLoginPage) {
      console.log("✅ Redirigé vers la page de login après expiration de session");
   ...
   */
  test("should handle multiple failed login attempts", async ({ page }, testInfo) => {
    test.skip(
    ...
    for (let i = 1; i <= numberOfAttempts; i++) {
      console.log(`🔄 Tentative ${i}/${numberOfAttempts} avec mauvais mot de passe`);
      ...
      const welcomeVisible = await page
        .getByRole("heading", { name: /Dashboard|Ton Dashboard/i })
        .isVisible()
        .catch(() => false);
...
test.describe("Auth Security - Additional Robustness Tests @p1 @security", () => {
   ...
   */
  test("should not expose password in network requests", async ({ page }, testInfo) => {
    await page.setExtraHTTPHeaders({
      "x-forwarded-for": getUniqueTestIp(testInfo.title, testInfo.file, testInfo.project.name),
    });
    ...
    console.log(`✅ ${requests.length} requêtes interceptées pour vérification de sécurité`);
  });

  /**
   * Vérifier que le champ mot de passe utilise type="password"
   */
  test("should use password input type for password field", async ({ page }, testInfo) => {
    await page.setExtraHTTPHeaders({
      "x-forwarded-for": getUniqueTestIp(testInfo.title, testInfo.file, testInfo.project.name),
    });
    await navigateToAdminLogin(page);
   ...
   */
  test("should disable submit button during login attempt", async ({ page }, testInfo) => {
    await page.setExtraHTTPHeaders({
      "x-forwarded-for": getUniqueTestIp(testInfo.title, testInfo.file, testInfo.project.name),
    });
    await navigateToAdminLogin(page);
    ...
    if (isDisabledDuringSubmit) {
      ...
      console.log("⚠️  Aucune protection visuelle détectée pendant la soumission");
    }
