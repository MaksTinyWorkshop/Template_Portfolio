...
test.describe
  .serial("Admin Availability Suite", () => {
    ...
    test.describe("Admin Availability Management - E2E Tests @p0 @admin @availability @e2e", () => {
       ...
       */
      test("should update availability status with toast confirmation", async ({
        page,
      }, testInfo) => {
        test.skip(
          testInfo.project.name !== "Desktop Chromium",
        ...
        if (!isVisible) {
          ...
          const newStatus: AvailabilityStatus =
            initialStatus === "available" ? "soon" : "available";
        ...
        const statusButtons = page.locator(
          'button:has-text("Disponible"), button:has-text("Bientôt"), button:has-text("Non disponible"), button:has-text("🟢"), button:has-text("🟡"), button:has-text("🔴")',
        );
