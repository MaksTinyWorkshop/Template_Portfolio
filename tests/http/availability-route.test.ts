...
describe("availability route", () => {
  ...
  it("renvoie la disponibilité par défaut quand aucun log n'existe", async () => {
    ...
    const { handleGetAvailability } = await import("@/app/(api)/api/availability/route");

    const response = await handleGetAvailability({} as NextRequest);
  ...
  it("bloque le POST quand on n'est pas authentifié", async () => {
    ...
    const { handlePostAvailability } = await import("@/app/(api)/api/availability/route");
    const response = await handlePostAvailability(buildRequest({ status: "available" }));
  ...
  it("renvoie 400 pour un statut invalide", async () => {
    ...
    const { handlePostAvailability } = await import("@/app/(api)/api/availability/route");
    const response = await handlePostAvailability(buildRequest({ status: "wrong" }));
  ...
  it("renvoie 404 en POST quand aucun proprietaire de site n'existe", async () => {
    ...
    const response = await handlePostAvailability(buildRequest({ status: "soon" }));
    expect(response.status).toBe(404);
    const payload = await response.json();
    expect(payload.error).toContain("propriétaire");
  });
  ...
  it("renvoie 500 quand la creation echoue", async () => {
    ...
    const payload = await response.json();
    expect(payload).toEqual({ error: "db down" });
  });
